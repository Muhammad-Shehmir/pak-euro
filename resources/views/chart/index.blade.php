                ...
                <div class="card mb-4">
                    ...
                    <div class="collapse" id="collapseExample">
                        <div class="d-grid p-3 border">
                            <form method="Get" id="myForm" action="{{ url('/chart-of-account') }}" enctype="multipart/form-data"
                                ...
                                <div class="col-md-3">
                                    <div class="input-group input-group-merge mb-2">
                                        ...
                                        <div class="form-floating form-floating-outline">
                                            <input name="mobile_no" type="text" id="basic-icon-default-phone"
                                ...
                                <div class="d-flex justify-content-end  p-1 py-1">
                                    ...
                                    <a href="{{ url('/chart-of-account') }}" class="btn btn-danger" tabindex="0"><span><i
        ...
        <div class="card">
            <div class="card-header flex-column flex-md-row">
                ...
                <div class="dt-action-buttons pt-3 pt-md-0">
                    <div class="dt-buttons btn-group flex-wrap">
                        <a href="{{ url('/chart-of-account/add') }}" class="btn btn-secondary create-new btn-primary" tabindex="0"
            ...
            <div class="card-datatable table-responsive pt-0 p-3">
                <table class="datatables-basic table table-bordered">
                    ...
                    <tbody>
                        @foreach ($chart_of_account as $chart)
                            <tr>
                                <td>{{ $chart->id }}</td>
                                <td>{{ $chart->name }}</td>
                                {{-- <td>{{ $chart->mobile_no }}</td>
                                <td>{{ $chart->cnic }}</td> --}}
                                <td>{{ $chart->account_type->name ?? '' }}</td>
                                {{-- <td>Account Type 2</td> --}}
                                {{-- <td>Class 2</td> --}}
                                {{-- <td>{{ $chart->category_id }}</td> --}}
                                {{-- <td>{{ $chart->account_type_id }}</td> --}}
                                {{-- <td>{{ $chart->class_id }}</td> --}}
                                <td>
                                    <div class="d-flex align-items-center">
                                        <a class="btn btn-primary me-2 p-2-5"
                                            href="{{ url('/chart-of-account/edit/' . $chart->id) }}"><i class="fa fa-edit"></i> </a>
                                        <form method="post" action="{{ url('/chart-of-account/delete/' . $chart->id) }}">
                                            @csrf
            ...
            <div class="card-footer d-flex justify-content-end">
                ...
                {{ $chart_of_account->links('pagination::bootstrap-4') }}
            </div>
        </div>
    </div>
    <script>
        function changePerPage(select) {
            ...
            window.location.href = `{{ url('chart-of-account') }}?perPage=${perPage}`;
        }
    </script>
@endsection
